...
public class hotelView {
	...
	public hotelView(hotelModel model) {
		...
		pane.add(getRegisterPanel(), "Register");import java.awt.CardLayout;
import java.awt.Color;
import java.awt.Font;
import java.awt.GridBagConstraints;
import java.awt.Insets;
...
/*	private String formatUsers(ArrayList<Account> users) {
	...
	private JPanel getSettingsPanel() {
		...
		panel.addLabel("Last Name: ", 12, "left", null, null, 0, 1);
		panel.addLabel("Password: ", 12, "left", null, null, 0, 3);
		panel.addLabel("Comfirm Password: ", 12, "left", null, null, 0, 4);
	...
	private JPanel getArchivePanel() {
		...
		grid.insets = new Insets(10,10,10,10);
		panel.addLabel("Enter a date. Reservations, room service requests, and "
				+ "complaints will be archived from this date.", 12, "left", null, null, 0, 1);
		...
		panel.addComponent(archiveButton, 0, 3);
		panel.addNavigation("Back to main menu", 12, "Manager", 0, 4);
		return panel;
	}
	...
	private JPanel getLoginPanel() {
		...
		grid.gridwidth = 1;
		panel.addNavigation("Register", 14, "Register", 0, 5);
	...
	private JPanel getRegisterPanel() {
		...
		panel.addNavigation("Back", 16, "Login", 0, 10);
		...
		if (role.equalsIgnoreCase("manager")) {
			...
			rating.addActionListener(new ActionListener(){
				...
				public void actionPerformed(ActionEvent e) {
					String[] rate = {"Select a rating", "1", "2", "3", "4" , "5"};
					JComboBox selectRate = new JComboBox(rate);
					int result = JOptionPane.showConfirmDialog(null, selectRate, "Rating of our overall servce", JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE);
					if (result == JOptionPane.OK_OPTION) {
						String r = (String)selectRate.getSelectedItem();
						if(r.equals("Select a rating")) {
							JOptionPane.showMessageDialog(new JFrame(), "Please select a rating.", "Error", JOptionPane.ERROR_MESSAGE);
			...
			panel.addNavigation("Handel room request", 16, "Tasks", 0, 1);
			panel.addComponent(settingButton, 0, 2);
		}
		return panel;
	}
	...
	private JPanel getViewCancelPanel() {
		...
		grid.ipady = 0;
		grid.insets = new Insets(10,10,10,10);
		panel.addLabel("Below are all your reservations. To cancel a \n"
				+ "reservation: Select the one you wish to cancel. Press cancel.\n"
				+ "If the list is empty, then you have not made any reservations.", 12, "left", 
				null, null, 0, 1);
	...
	private JPanel getReservationsPanel() {
		...
		grid.gridwidth = 2;
		JButton roomButton = new JButton("Order by Rooms");
		roomButton.setFont(new Font("Tahoma", Font.BOLD, 12));
		panel.addComponent(roomButton, 0, 3);
		JButton customerButton = new JButton("Order by Customer");
		roomButton.setFont(new Font("Tahoma", Font.BOLD, 12));
		panel.addComponent(customerButton, 2, 3);
	...
	private JPanel getUsersPanel() {
		...
		grid.gridwidth = 1;
		JButton addEmployeeButton = new JButton("Add Employee");
		addEmployeeButton.setFont(new Font("Tahoma", Font.BOLD, 12));
		panel.addComponent(addEmployeeButton, 0, 2);
		addEmployeeButton.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				switchPanel("AddEmployee");
	...
	private JPanel getFileComplaintPanel() {
		...
		grid.ipady = 0;
		grid.insets = new Insets(10,10,10,10);
		panel.addLabel("We apologize for any inconvenience.\n"
				+ "Please file your complaint here and a hotel manager\n"
				+ "will contact you as soon as possible.", 12, "left", null, null, 0, 1);
		...
		grid.weighty = 0;
		panel.addNavigation("Back", 16, "Customer", 0, 3);
	...
	private JPanel getRoomServicePanel() {
		...
		panel.addComponent(submitButton, 0, 3);
		panel.addNavigation("Back to main menu", 12, "Room Attendant" , 0, 4);
		return panel;
	}
	...
	private JPanel getOrderRoomServicePanel() {
		...
		grid.ipady = 0;
		grid.insets = new Insets(10,10,10,10);
		panel.addLabel("Description: (Please, include your room number in the description)", 12, "left", null, null, 0, 1);
